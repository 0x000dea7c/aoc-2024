...
inline uint32_t parse_int(char const *str) {
  uint32_t n = 0;
  while (str && *str >= '0') {
    n = n * 10 + (*str++ - '0');
  }
...
int main() {
  ...
  std::array<uint32_t, MAX_NUM> list2 =
      {}; // already know the max number if the input list, so this allows for a
          ...
          // for faster lookups
  {
    ...
    while ((bytes_read = read(fd, buffer, BUFF_SIZE)) >
           0) { // reading chunks is way better than char by char
      for (size_t i = 0; i < bytes_read; ++i) {
        char c = buffer[i];
        // parsing numbers by myself is way faster than std::sscanf
        if (c >= '0' && c <= '9') {
